...
require_once "functions_front.php";
 $table="items";
 $from="1";
 $searchoption='<button id="dropdownbtn" data="title" type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                标题
...
<body>
  ...
  <input type="text" id="table" value="items" class="hidden">
  <input type="text" id="from" value="1" class="hidden">
    ...
    $(function(){
      ...
      var pullupAction = function(){
        ...
        if(typeof($col)=="undefined" || typeof($key)=="undefined")
          $col=$key="";
        $.getJSON('front_get.php', {table:"items",from:1,page:page,pagesize:10,col:$col,key:$key}, function(json) {
              if (json.length>0) {
                $new=false;
                 ...
                 // console.log(val.uploaddate);
                  ...
                  if ($content.length>0) {
                    ...
                    $tmp='<section><span class="point-time '+(val.isfounded>0?"point-green":"point-yellow")+'"></span><aside data="'+val.ID+'"><p class="title">'+val.title+'</p><p class="brief"><span class="text-white"><b>描述:</b>'+subdescription(val.description)+'</span></p></aside></section>';
                    $content.append($tmp);
                  }else{
                    $new=true;
